...
def search(L, pl, pr, tl, tr, lm, rm, rep):
    ...
    if flag:
        ...
        if (pattern[0][0] == '|' or target[0][0] == '|'):
            lhs = False
        if (pattern[0][-1] == '|' or target[0][-1] == '|'):
            rhs = False
        ...
        if(lhs or rhs):
            ...
            pattern, target, pattern_location = search(
                L, pl, pr, tl, tr, lhs, rhs, rep+1)  # recursion call
        else:
...
def discover_maximal_repeat(L):
        ...
        for l in range(len(idx)):  # pi = the position of pattern
            ...
            for ti in indice[1:]:  # ti = the position of target
                ...
                if maximal_repeat_set.get(repeated_activity, False) == False:
                    maximal_repeat_set[repeated_activity] = {
                ...
                # if already exist in the dic.
                    ...
                    for location in pattern_location:
                        if location not in maximal_repeat_set[repeated_activity]['location']:
                            maximal_repeat_set[repeated_activity]['location'].append(
                                location)
    ...
    return list(maximal_repeat_set.keys())
...
#     maximal_repeat_set.sort(key=len)
    ...
    super_maximal_repeat_set += discover_super_maximal_repeat(
        maximal_repeat_set)
...
def pattern_discover(log):
    ...
    super_maximal_repeat_set = discover_super_maximal_repeat(
        maximal_repeat_set_)
    near_super_maximal_repeat_minus_sm = discover_near_super_maximal_repeat_minus_sm(
        dic_mr_set, maximal_repeat_set, super_maximal_repeat_set)
    near_super_maximal_repeat = near_super_maximal_repeat_minus_sm + \
        super_maximal_repeat_set
